...
preds_box: 検出矩形のアンカーボックスからの誤差[バッチサイズ数、アンカーボックス数、 4(x_diff, y_diff, w_diff, h_diff)]
...
def loss_func(preds_class, preds_box, anchors, targets, iou_lower_threshold = 0.4, iou_upper_threshold = 0.5):
    ...
    for img_preds_class, img_preds_box, img_targets in zip(preds_class, preds_box, targets):
        ...
        #IoUがしたの閾値と上の閾値の間にあるアンカーボックスについては、分類の損失を計算しない
        loss_class += ((targets_class != -1) * sigmoid_focal_loss(img_preds_class, targets_class)).sum() / num_positive_anchors.clamp(min = 1)

        #陽性のアンカーボックスが1つも存在しない時、矩形の誤差は計算しない
        if num_positive_anchors == 0:
            continue

        #各アンカーボックスにマッチした正解矩形を抽出
        assinged_boxes = img_targets['boxes'][ious_argmax]
        assinged_boxes_xywh = util.convert_to_xywh(assinged_boxes)

        #アンカーボックスとマッチした正解矩形との誤差を計算し、ラベルを作成
        targets_box = torch.zeros_like(img_preds_box)
        #中心位置の誤差はアンカーボックスの大きさでスケール
        targets_box[:, :2] = (assinged_boxes_xywh[:, :2] - anchors_xywh[:, :2]) / anchors_xywh[:, 2:]
        targets_box[:, 2:] = (assinged_boxes_xywh[:, 2:] / anchors_xywh[:, 2:]).log()
